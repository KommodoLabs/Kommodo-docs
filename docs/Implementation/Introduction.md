...
Kommodo is a permissionless, on-chain lending protocol built on Ethereum that integrates directly with Uniswap V3 liquidity pools. It enables lenders to earn yield on their assets by supplying liquidity at specific price ticks, and allows borrowers to take collateralized loans against that liquidity. Any token pair with an existing Uniswap V3 pool can be used as a lending market.
...
| Deployment target | Ethereum (EVM-compatible chains) |
