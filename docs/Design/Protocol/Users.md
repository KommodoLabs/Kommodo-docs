...
As time progresses the borrowers interest is consumed and when no more interest is available the loan is locked (in default). After the lock no more adjustments are possible and anyone can close the loan. The account that closes the loan receives the margin deposit as a reward for closing. Since the loan is fully closed no solvency check is performed.